...
def mauve(dirpath, output, backbone,location_mauve):
    ...
    for x in xrange(len(file_list)):
        systemCatll = "%s %s" %(systemCall, file_list2[x])
...
# Extracts best model from JModelTest's output
...
def aligned_sequence(dirpath, output, backbone, location_mauve, location_jModelTest, output_model):
    ...
    print get_best_model("%s" %(output_model))
...
## Web app not working yet. Using localhost paths. (assuming user downloads our entire files)
