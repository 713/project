...
def run_kr(fasta_file,kr_path):
    ...
    for i in range(n):
        for j in range(n):
            if((dict[(listSeq[i],listSeq[j])] > 0) ) and (dict[(listSeq[i],listSeq[j])] < 0.0045):  #threshold: 0.004
...
def find_HGT(fasta_path,alfy_location,output_folder):
    ...
    ##############################################################################
    Dict = run_kr(fasta_path+"/allSeq.fasta","/afs/andrew.cmu.edu/usr24/yimingx/private/03713/Kr_2.0.2/Src/Kr")
    #print Dict
    HGT_dict = dict()
